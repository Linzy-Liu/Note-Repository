...
$$y_{n+k} = \sum_{i=0}^{k-1} \alpha_i y_{n+i}+h\sum_{i=0}^{k}f_{n+i}$$其中$f_{n+i}$表示$f(x_{n+i},y_{n+i})$。
在这一形式下，线性多步法的截断误差可以表示为：$$T_{n+k} = y(x_{n+k}) - \sum_{i=0}^{k-1} \alpha_i y(x_{n+i})+h\sum_{i=0}^{k}y'(x_{n+i})$$
...
考虑以下形式的公式：
$$y_{n+3} = \alpha_1y_{n+1}+\alpha_2y_{n+2}+h(\beta_1f_{n+1}+\beta_2f_{n+2}+\beta_3f_{n+3})$$
（因为要找比Simpson的更为稳定的变体，故不能取$\alpha_1 = 1$）不妨令$\alpha_1=0$通过泰勒展开，可以解得一组系数，代入便可以得到汉明（Hamming）方法：
$$y_{n+3} = \frac{1}{8}(9y_{n+2}-y_n) + \frac{3h}{8}(f_{n+3}+2f_{n+2}-f_{n+3})$$
这一方法是一个隐式三步四阶方法
