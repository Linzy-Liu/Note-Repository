...
注意到，牛顿法不总是收敛的，为了让迭代式兼具收敛和快速的特点，我们尝试将牛顿法和下山法结合在一起。所谓下山法，即引入迭代序列单调的条件的方法。在求根中，则表现为函数值与0的距离单调下降：
...
$$y_{n+k} = \sum_{i=0}^{k-1} \alpha_i y_{n+i}+h\sum_{i=0}^{k}\beta_if_{n+i}$$其中$f_{n+i}$表示$f(x_{n+i},y_{n+i})$。
在这一形式下，线性多步法的截断误差可以表示为：$$T_{n+k} = y(x_{n+k}) - \sum_{i=0}^{k-1} \alpha_i y(x_{n+i})+h\sum_{i=0}^{k}\beta_iy'(x_{n+i})$$

由上面的敛散性定理的证明过程可以同理推出：线性多步法收敛的充要条件是：
$$
\left\{
\begin{align*}
  \sum_{i=0}^{k-1} \alpha_i &= 1\\
  \sum_{i=0}^{k-1} i\alpha_i + \sum_{i=0}^k \beta_i &= k
\end{align*}
\right.
$$
...
不妨对Adams方法稍作更改，则有形如$$
y_{n+k} = y_n+h\sum_{i=0}^{k-1} \beta_if_{n+i}
$$的$k$步法，令$k=4$时则为**Milne法**：$$
y_{n+4} = y_n+\frac{4h}{3}(2f_{n+3} - f_{n+2}+2f_{n+1})
$$
可以认为这一方法是来自于积分
$$y(x_{n+4})-y(x_n)=\int_{x_n}^{x_{n+4}} f(x,y(x))\,dx$$那么利用Simpson公式同理有
$$y(x_{n+2})-y(x_n)=\int_{x_n}^{x_{n+2}} f(x,y(x))\,dx$$
即$$y_{n+2} = y_n+\frac{h}{3}(f_n + 4f_{n+1}+ f_{n+2})$$这一公式被称为**Simpson方法**，它和Milne方法均为四阶方法。

### 7.5.3 Hamming方法

Simpson是二阶方法中精度最高的，但相当不稳定。为了改进这一缺陷，我们尝试探索三步方法中的一些选择。
考虑以下形式的公式：
$$y_{n+3} = \alpha_0y_n+\alpha_1y_{n+1}+\alpha_2y_{n+2}+h(\beta_1f_{n+1}+\beta_2f_{n+2}+\beta_3f_{n+3})$$
（因为要找比Simpson的更为稳定的变体，故不能取$\alpha_1 = 1$）不妨令$\alpha_1=0$通过泰勒展开，可以解得一组系数，代入便可以得到汉明（Hamming）方法：
$$y_{n+3} = \frac{1}{8}(9y_{n+2}-y_n) + \frac{3h}{8}(f_{n+3}+2f_{n+2}-f_{n+1})$$
这一方法是一个隐式三步四阶方法
