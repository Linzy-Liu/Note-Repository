...
显然，这一指标可以用于刻画经历$n$步迭代后，所求数值解的误差上限。而回顾我们已经介绍过的数值方法，可以易于证明：欧拉法和后退欧拉法具有1阶精度，梯形法具有二阶精度。

## 7.3 龙格-库塔法

### 7.3.1 显式R-K法的一般式

事实上，这一方法来自于对$y(x_{n+1})-y(x_n) = \int_{x_n}^{x_{n+1}} f(t,y(t))\,dt$右侧积分的精确计算。这里我们直接考虑列出R-K法的一般式：
